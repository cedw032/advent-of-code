...
import Data.Array (filter, head, index, last, length)
...
main :: Effect Unit
main = do
  log $ show $ length $ getValidPaswords2 $ prepareInput input

-- log $ show $ length $ getValidPaswords1 $ prepareInput input
getValidPaswords2 :: Array PasswordInfo -> Array PasswordInfo
getValidPaswords2 a = filter isValid a
  where
  isValid :: PasswordInfo -> Boolean
  isValid p =
    charIsAtMax p && not charIsAtMin p
      || charIsAtMin p && not charIsAtMax p

  charIsAtMax :: PasswordInfo -> Boolean
  charIsAtMax p = (charAt' (p.maximum - 1) p.password) == p.character

  charIsAtMin :: PasswordInfo -> Boolean
  charIsAtMin p = (charAt' (p.minimum - 1) p.password) == p.character

  charAt' :: Int -> String -> Char
  charAt' i s = fromMaybe '-' $ charAt i s

getValidPaswords1 :: Array PasswordInfo -> Array String
getValidPaswords1 a = do
  p <- a
  guard $ isValid p
  [ p.password ]
  where
  isValid :: PasswordInfo -> Boolean
  isValid p' =
    isBetween p'.minimum p'.maximum
      $ length
      $ filter (\c -> c == p'.character)
      $ toCharArray p'.password

  isBetween :: Int -> Int -> Int -> Boolean
  isBetween minimum maximum i = i >= minimum && i <= maximum
...
prepareInput :: String -> Array (PasswordInfo)
prepareInput s = toPasswordInfo <$> splitLines <$> splitBlock s
  where
  splitBlock :: String -> Array String
  splitBlock block = split (Pattern "\n") block

  splitLines :: String -> Array String
  splitLines lines = split (Pattern " ") lines

  toPasswordInfo :: Array String -> PasswordInfo
  toPasswordInfo parts =
    { minimum: extractMinimum parts
    , maximum: extractMaximum parts
    , character: extractCharacter parts
    , password: defaultEmptyString $ index parts 2
    }

  defaultEmptyString :: Maybe String -> String
  defaultEmptyString ms = fromMaybe "" ms

  extractCharacter :: Array String -> Char
  extractCharacter a =
    fromMaybe '_'
      $ toChar
      $ take 1
  ...
  toInt :: String -> Int
  toInt s = fromMaybe 0 $ fromStringAs decimal s

  extractMinimum :: Array String -> Int
  extractMinimum a =
    toInt
      $ defaultEmptyString
      $ head
  ...
  extractMaximum :: Array String -> Int
  extractMaximum a =
    toInt
      $ defaultEmptyString
      $ last
  ...
  extractRange :: Array String -> String
  extractRange a = defaultEmptyString $ index a 0
